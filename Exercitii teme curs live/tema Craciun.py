...
def sum_of_values(dict11):

    maximvalues= sum(dict1.values())
    return maximvalues
...
# and the budget (min & max) and return all the phones which are in the price inter
...
# prima partea din exercitiu max account rezolvare
max_account = None
 ...
 # partea 2 din exercitiu diferenta dintre account si debt rezolvare
max_diff = None
 for user in accounts:
  if not max_diff:
        max_diff = user
